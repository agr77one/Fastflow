...
- version: `2.5.2` = FLM env repair + autostart repair; repo `agr77one/Fastflow`
...
B51|2026-08-27|live: ∀ hotkey + warmup failed w/ only `FastFlowLM server exited early (exit 1).` — 5 failures, ZERO diagnosable signal, ∵ `start_flm_server` passed `stdout/stderr=None` (inherit) when `log_to_file`=false (the default) ∴ FLM's own error was discarded. Real cause was specific ∧ actionable all along: `Error: create_directories: Access is denied.: "C:\Windows\system32\config\systemprofile\.flm"` (FLM resolving its model dir to the SYSTEM profile when spawned by the daemon; reproduces daemon-only, ⊥ from a normal shell). Cost hours of bisecting ∵ the evidence existed ∧ was thrown away|V63; ALWAYS capture provider output — scratch file when `log_to_file`=false (deleted iff start ok), persistent log otherwise; on early-exit ∨ timeout append ANSI-stripped bounded tail to the RuntimeError. Live-verified BOTH paths: error now names the access-denied cause. NB the daemon-vs-shell context difference itself is STILL UNDIAGNOSED — ruled out interpreter, Store `pyw.exe` shim, creationflags, console handles, AHK env edits, elevated shortcut, missing USERPROFILE/HOME
B54|2026-08-31|THE actual root cause of the "exited early (exit 1)" saga, found only once B51 surfaced provider output + `doctor` reported env: FLM's installer stores `FLM_MODEL_PATH` as a **MACHINE-scope REG_EXPAND_SZ** holding `%USERPROFILE%\.flm`. Machine-scope vars expand in the SYSTEM context when a session env block is built ∴ every inheriting process saw `C:\Windows\system32\config\systemprofile\.flm` → `flm` died on `create_directories: Access is denied`. Explains ∀ observations: fails from the app (explorer→AHK→daemon→flm inherit the poisoned block), works from an interactive shell (fresh expansion), survives reboot, ⊥ fixed by FLM 1.0.3. UPSTREAM installer bug (per-user path in a machine-scope var). MY ERROR: after 1.0.3 I declared it "solved" on one passing Stop→Start ∵ a server was already warm; the user's reboot disproved it|V66; `ffp_flm_server.flm_env()` repairs the value for every `flm` child (serve/list/version/pull/remove/bench); live-verified cold Start 1.1s + hotkey OK w/ the env still poisoned
